...
function Sys573.FlashMemory.twcTimerCB()
	local current_cycles = PCSX.getCPUCycles()
	if (current_cycles >= Sys573.FlashMemory.m_twcEndCycle) then
		--print('10ms timer expired, missed by ' .. string.format("%x",(current_cycles - Sys573.FlashMemory.m_twcEndCycle) / (PCSX.CONSTS.CPU.CLOCKSPEED / 1000)) .. 'ms')
		Sys573.FlashMemory.m_pageWriteEnabled = false
		Sys573.FlashMemory.m_targetWritePage = -1
		PCSX.nextTick(
			function()
				Sys573.FlashMemory.m_twcTimer = Sys573.FlashMemory.m_twcTimer:remove()
			end
		)
	end
end

function Sys573.FlashMemory:tickTWCTimer()
	local current_cycles = PCSX.getCPUCycles()
...
function Sys573.FlashMemory:init()
...
function Sys573.FlashMemory:writeCommandBus(address, value)
...
function Sys573.FlashMemory:write8(address, value)
...
function Sys573.FlashMemory:softwareDataProtectEnablePageWrite()
	self.m_dataProtectEnabled = true
	self.m_pageWriteEnabled = true
	self:tickTWCTimer()
end

function Sys573.FlashMemory:softwareDataProtectDisable()
	self.m_dataProtectEnabled = false
end

function Sys573.FlashMemory:softwareChipErase()
	ffi.fill(PIOCart.m_cartData, 256 * 1024, 0xff)
end

function Sys573.FlashMemory:enterSoftwareIDMode()
	self:setLUTSoftwareID()
	self:resetCommandBuffer()
	self.m_softwareIDMode = true
end

function Sys573.FlashMemory:exitSoftwareIDMode()
	self:setLUTNormal()
	self:resetCommandBuffer()
	self.m_softwareIDMode = false
end

function Sys573.FlashMemory:checkCommand()
	local result = false
...
function Sys573.FlashMemory:resetCommandBuffer()
	ffi.fill(self.m_commandBuffer, 6, 0)
	ffi.fill(self.m_commandBufferAddress, 6, 0)
end

function Sys573.FlashMemory:resetFlash()
	self:resetCommandBuffer()
	self.m_dataProtectEnabled = true
	self.m_pageWriteEnabled = false
	self.m_softwareIDMode = false
...
function Sys573.FlashMemory:setLUTs()
...
function Sys573.FlashMemory:setLUTNormal()
...
function Sys573.FlashMemory:setLUTSoftwareID()
	local readLUT = PCSX.getReadLUT()
